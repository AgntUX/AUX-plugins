...
`reference/sync.md` must be ≤ 640 lines (the canonical procedural body is
~474 lines and needs headroom for a plugin's source-specific splices,
including the mandatory `step-reconcile-append.md`),
...
| 14 | E29 (warning) | **View-tool response envelope present**: every `view-tool/src/*-view.ts` MUST call `renderConfirmationText(…)` so each handler return ships a `content[].text` block alongside `structuredContent`. Without it the model treats the tool response as raw data and builds a duplicate widget + chat commentary summarizing the iframe the user can already see (Cowork post-render regression, 2026-05-18). Warning-only; routed to `view-tool-builder`. |
| 15 | E30 (warning) | **Grounded-tests guard**: flags a plugin test that both reads a `reference/`-dir `.md` file AND calls `.toContain(…)` — a brittle "phantom-contract" assertion that greps another author's per-plugin prose for an invented string. This is the multi-round test-churn class from the 2026-06-01 calendar build (`draft-flow` + `thread-association` both grepped the same `fetch.md`, so fixing one broke the next). Routed to `tests-author`; the fix is to assert handler output / a parsed `plugin.json`/`listing.yaml` field / a canonical `sync.md` anchor instead (see `plugins/agntux-build/agents/tests-author.md` → golden rule). Warning-only. Pairs with the gate change that now batches ALL failing vitest assertions into one verdict (`stage_results[].errors[].test_findings[]`) so the whole suite is fixed in one round rather than one-assertion-per-round. |
| 16 | E31 (error) | **View-tool external-link guard**: hard-fails any `view-tool/src/**` component (`.ts`/`.tsx`, excluding `lib/` + `__tests__/`) that carries `target="_blank"`, `window.open(`, or `<a href>`. Sandboxed MCP App iframes silently block raw anchor navigation and `window.open`, so the click is a dead no-op — the agntux-google-calendar 2026-06 dead-button class (4 "Open in Google Calendar" / "Join Google Meet" / "Sources" links that did nothing). Fix: render the shared `ExternalLink` component (`view-tool/src/components/external-link.tsx`, scaffolded by the agntux-build `_template`), which wraps `useAppsClient().openLink(url)` → emits the host's `ui/open-link` postMessage; or call `openLink()` directly from a `<button onClick>`. Routed to `ui-handler-author` (§1.7). The comment-scrubber preserves string literals + newlines, so the `ExternalLink` source — whose only `<a href>` mentions live in `//` comments — passes naturally. |
| 17 | E32 (error) | **View-tool connector-call guard**: hard-fails any `view-tool/src/**` component (same scope as pass 16) that calls `client.callTool("mcp__…")` with a hard-coded **non-`agntux`** (connector) tool name. Connector tool names are host-specific — UUID-prefixed in local agent mode (`mcp__<uuid>__create_event`), `mcp__claude_ai_<Connector>__create_event` on claude.ai — so a literal name throws `MCP error -32602: Tool not found` at click time (the agntux-google-calendar 2026-06 Schedule/Send "Tool not found: create_event" class). Fix: dispatch connector writes via `client.sendFollowUpMessage(envelope)` so the host's LLM resolves the connector tool; for connector reads whose result must reach the iframe, instruct the host to run the read and re-open the view pre-populated. Only the plugin's own `mcp__agntux…` action-mutation server tools may be called directly. Routed to `draft-flow-author`; see `plugins/agntux-build/canonical/prompts/ui/connector-envelopes.md`. |
| 18 | E33 (error) | **No slash-command host prompts**: hard-fails a slash command sent to the host *programmatically* — a `host_prompt:` value beginning with `/` in any `plugins/*/skills/**/*.md` (rendered or `_overrides/` source), a `sendFollowUpMessage("/…")` arg, or a hyphenated plugin-slug slash trigger phrase (`` `/agntux-slack …` ``) in a `view-tool/src` `description`. The host only fires a slash command when the **user manually types `/`**; one dispatched by agntux-core's hub (which forwards `host_prompt` verbatim) or matched against a description is inert text it can't route, so the button silently does nothing (the 2026-06-15 "Jira action button does nothing" class). Fix: write a natural-language description — `host_prompt: "Use the {slug} plugin to {imperative} for action {id}"`. Does **not** flag `ui://` resource URIs (mid-string slash), bare JSX display text (`Run /agntux onboard`), user-typed-command prose, or `listing.yaml` `supported_prompts` (the required user-typed slash surface). Routed to `ingest-prompt-author` / `ui-handler-author`. |
| 19 | E34 (error) | **View payload-coverage guard**: hard-fails a `## <View> payload` body section a `view-tool/src` handler READS (`extractFencedYaml`/`parseYamlSection`/`parseSectionYaml` 2nd arg, `…payload`-named) that the plugin's ingest skill never WRITES — i.e. the name is absent from the rendered `skills/<slug>/reference/sync.md`. Without the Step 10 write instruction the action file lacks the section and the view renders an empty envelope (the 2026-06-15 google-calendar "Untitled event" + every Jira view "… data is unavailable" class). Fix: add a per-view `_overrides/step-10-append.md` telling the skill to write `## <Name> payload` whenever the triggering suggested action ships, citing its `reference/<view>-payload.md` shape, then re-render. Skips hub-only plugins with no `sync.md`. Routed to `ingest-prompt-author`. |
| 20 | E35 (warning) | **View payload FIELD-coverage guard**: tightens E34 from heading-coverage to field-coverage. Collects the field keys a `view-tool/src` handler reads off an on-disk payload object (member access / destructuring of the variable returned by `parseYamlSection`/`parseSectionYaml`/`parseComposeSectionYaml`/`parseYaml`/`extractFrontmatterMetadata` or `parsed.compose_payload`), and warns on any field absent from the plugin's whole rendered skill tree (`sync.md` + `reference/*.md`). Catches the **apple-notes class**: the view read `draft_title`/`draft_body`/`target_folder`, but the plugin inherited the generic reply-centric schema (`drafted_body`/`thread_context`) and shipped no compose-payload override, so the ingest agent never wrote the note fields and the iframe rendered blank — E34 passed because the `## Compose payload` *heading* still matched. Fix: add the read fields to `_overrides/reference/compose-payload.md` (or per-view `*-payload.md`) AND a `_overrides/step-10-append.md` pre-compose instruction, then re-render. Warning (heterogeneous extraction → best-effort static linkage); promote to error once every plugin's field set is clean. Routed to `ingest-prompt-author`. |
| 21 | E36 (warning) | **Reconcile-declared guard**: every plugin that ships a `view-tool/` (it raises actionable items) AND an ingest `sync.md` must ship a non-empty `skills/<slug>/_overrides/step-reconcile-append.md` declaring this source's reconciliation signals (the per-source "Resolved when / Changed-but-valid when / Re-check via" list spliced at canonical sync.md's `<!-- append:step-reconcile -->` marker). Without it, Step 8.5 ("Reconcile open action items against fresh data") has only the generic skeleton and can't recognise the source's terminal/changed states, so handled items stay `open` and noisy. Fix: author the append per the `reference/reconcile.md` shape, then re-render. Warning so the cross-plugin sweep lands incrementally; promote to error once every action-producing plugin ships it. Routed to `ingest-prompt-author` / `source-semantics-advisor`. |

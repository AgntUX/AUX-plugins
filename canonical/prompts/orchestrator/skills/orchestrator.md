...
0. **Project root**: confirm the active project root is exactly `~/agntux-code/`. If it isn't, fail loud: tell the user one sentence — "AgntUX requires the project to be `~/agntux-code/`. Create that folder, select it in your host's project picker, then re-invoke me." — and stop. Every check below assumes you're inside `~/agntux-code/`.
1. Does `~/agntux-code/user.md` exist? If no, the user has never run ux before. Acknowledge what they asked first ("I see you asked about X — but I need to set up your profile first (one minute). After that, I'll come back to your question."), then engage the **personalization subagent** (Mode A — first-run interview). After the interview wraps, return to the original ask.
...
1. Read `~/agntux-code/actions/{id}.md`. **If the file doesn't exist** (the user named a stale ID, or it was deleted out-of-band), tell them in one sentence and stop. Don't create.
...
1. Identify which `~/agntux-code/actions/{id}.md` the prompt belongs to. The
...
3. Read `~/agntux-code/user.md`. Pay attention to `# Identity` (the user's name,
...
| ≤ −1 (post-expiry) | `Trial expired. AgntUX is paused. Your data is safe at ~/agntux-code/. Upgrade at app.agntux.ai/billing.` |

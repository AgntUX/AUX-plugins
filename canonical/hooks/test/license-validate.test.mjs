...
test("decide: file Write under <agntux project root>/ with valid cache -> allow", () => {
  const { dir } = setup();
  try {
    const claims = defaultClaims();
    const cached = { token: mintJwt(claims), expires_at: claims.exp };
...
test("decide: file Write outside <agntux project root>/ -> allow (out of scope) regardless of cache", () => {
  const { dir } = setup();
  try {

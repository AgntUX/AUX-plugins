...
the next run the world has moved: the Dropbox artefact may
...
- **Re-check via** — the `list_folder, list_folder/continue, search, get_file_metadata, fetch, file_preview, list_shared_links, get_shared_link_metadata, list_file_requests, get_file_request, download_link` read call(s) that fetch the
...
to Dropbox. A re-check failure logs
`dropbox-reconcile-failed` and skips that candidate; Step 11's

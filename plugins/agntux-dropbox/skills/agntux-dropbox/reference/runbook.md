# Dropbox sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `dropbox-merged-into-{existing_id}`,
  `dropbox-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `dropbox-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `dropbox-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `dropbox-deferred-orphan: <id>` (the originating
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `dropbox-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `dropbox-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-dropbox/sync.md → errors` with one of

# Natural-language query against Dropbox

The user typed `/agntux-dropbox <words>` where the first token is
not `sync`. They want a live answer about Dropbox.
...
may not exist, and that's fine. The Dropbox read MCP
...
1. Use these Dropbox read MCP tools to investigate:
   list_folder, list_folder/continue, search, get_file_metadata, fetch, file_preview, list_shared_links, get_shared_link_metadata, list_file_requests, get_file_request, download_link.
...
configured for dropbox), reply verbatim:

> "I can't reach Dropbox right now. Make sure the
> Dropbox connector is enabled in your host's

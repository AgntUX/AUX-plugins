---
name: agntux-dropbox
description: The Dropbox command surface for AgntUX. Runs an ingest pass against Dropbox or answers natural-language questions about dropbox content live. Use for "/agntux-dropbox", "sync dropbox", "ingest dropbox now", "refresh dropbox", "fire a dropbox pass", "what's happening in #Work Projects", "what's in my dropbox", "what did {person} say in dropbox this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread dropbox", or any Dropbox-scoped question.
...
# `/agntux-dropbox` — Dropbox command surface

Lane: single user-facing entry into the agntux-dropbox plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Dropbox surface of AgntUX. Never reference

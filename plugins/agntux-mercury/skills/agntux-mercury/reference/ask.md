# Natural-language query against Mercury

The user typed `/agntux-mercury <words>` where the first token is
not `sync`. They want a live answer about Mercury.
...
may not exist, and that's fine. The Mercury read MCP
...
1. Use these Mercury read MCP tools to investigate:
   getAccounts, getAccount, listTransactions, getTransaction, getTransactionById, getAccountCards, listCredit, getTreasury, getTreasuryTransactions, getTreasuryStatements, listSendMoneyApprovalRequests, getSafeRequests, listInvoices, getInvoice, listInvoiceAttachments, getRecipients, getRecipient, listCustomers, getCustomer, getUsers, getOrganization, listCategories, getAccountStatements.
...
configured for mercury), reply verbatim:

> "I can't reach Mercury right now. Make sure the
> Mercury connector is enabled in your host's

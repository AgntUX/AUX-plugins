# Mercury sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `mercury-merged-into-{existing_id}`,
  `mercury-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `mercury-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `mercury-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `mercury-deferred-orphan: <id>` (the originating
  transaction's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `mercury-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `mercury-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-mercury/sync.md → errors` with one of

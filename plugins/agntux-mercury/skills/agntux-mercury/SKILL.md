---
name: agntux-mercury
description: The Mercury command surface for AgntUX. Runs an ingest pass against Mercury or answers natural-language questions about mercury content live. Use for "/agntux-mercury", "sync mercury", "ingest mercury now", "refresh mercury", "fire a mercury pass", "what's happening in #checking", "what's in my mercury", "what did {person} say in mercury this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread mercury", or any Mercury-scoped question.
...
# `/agntux-mercury` — Mercury command surface

Lane: single user-facing entry into the agntux-mercury plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Mercury surface of AgntUX. Never reference

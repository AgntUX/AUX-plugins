// render-reproducibility.test.ts — mirrors lint pass 8 (E15) for agntux-mercury.
//
// Asserts the rendered tree at skills/agntux-mercury/ is byte-identical to
// what node scripts/render-skill.mjs would produce from the committed
...
//
// The test locates the render-skill.mjs via CLAUDE_PLUGIN_ROOT (the installed
// agntux-build bundle root; set by agntux_validate). On a cold developer
// machine without the env var, the test is skipped rather than hard-failing
// so it never blocks a local `vitest run`.
...
function findRenderSkillScript(): string | null {
  ...
  // up from the build date dir, but only if the layout happens to match).

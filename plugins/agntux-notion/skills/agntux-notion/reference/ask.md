# Natural-language query against Notion

The user typed `/agntux-notion <words>` where the first token is
not `sync`. They want a live answer about Notion.
...
may not exist, and that's fine. The Notion read MCP
...
1. Use these Notion read MCP tools to investigate:
   notion-search, notion-fetch, notion-get-comments, notion-query-database-view, notion-query-data-sources, notion-query-meeting-notes, notion-get-users, notion-get-teams.
...
configured for notion), reply verbatim:

> "I can't reach Notion right now. Make sure the
> Notion connector is enabled in your host's

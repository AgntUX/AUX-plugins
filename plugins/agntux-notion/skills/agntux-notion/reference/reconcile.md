...
the next run the world has moved: the Notion artefact may
...
- **Re-check via** — the `notion-search, notion-fetch, notion-get-comments, notion-query-database-view, notion-query-data-sources, notion-query-meeting-notes, notion-get-users, notion-get-teams` read call(s) that fetch the
...
to Notion. A re-check failure logs
`notion-reconcile-failed` and skips that candidate; Step 11's

...
- **Cross-source dedup outcomes** (Step 9) —
  `notion-merged-into-{existing_id}`,
  `notion-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `notion-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `notion-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `notion-deferred-orphan: <id>` (the originating
  page's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `notion-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `notion-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-notion/sync.md → errors` with one of

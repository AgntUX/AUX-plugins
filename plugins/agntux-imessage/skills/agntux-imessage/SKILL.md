---
name: agntux-imessage
description: The iMessage command surface for AgntUX. Runs an ingest pass against iMessage or answers natural-language questions about imessage content live. Use for "/agntux-imessage", "sync imessage", "ingest imessage now", "refresh imessage", "fire a imessage pass", "what's happening in #Mom", "what's in my imessage", "what did {person} say in imessage this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread imessage", or any iMessage-scoped question.
...
# `/agntux-imessage` — iMessage command surface

Lane: single user-facing entry into the agntux-imessage plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the iMessage surface of AgntUX. Never reference

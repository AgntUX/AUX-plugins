# iMessage sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `imessage-merged-into-{existing_id}`,
  `imessage-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `imessage-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `imessage-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `imessage-deferred-orphan: <id>` (the originating
  thread's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `imessage-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `imessage-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-imessage/sync.md → errors` with one of

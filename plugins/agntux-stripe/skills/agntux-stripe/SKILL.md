---
name: agntux-stripe
description: The Stripe command surface for AgntUX. Runs an ingest pass against Stripe or answers natural-language questions about stripe content live. Use for "/agntux-stripe", "sync stripe", "ingest stripe now", "refresh stripe", "fire a stripe pass", "what's happening in #payment", "what's in my stripe", "what did {person} say in stripe this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread stripe", or any Stripe-scoped question.
...
# `/agntux-stripe` — Stripe command surface

Lane: single user-facing entry into the agntux-stripe plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Stripe surface of AgntUX. Never reference

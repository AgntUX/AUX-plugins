...
the next run the world has moved: the Stripe artefact may
...
- **Re-check via** — the `search_stripe_resources, fetch_stripe_resources, stripe_api_read, stripe_api_search, stripe_api_details` read call(s) that fetch the
...
to Stripe. A re-check failure logs
`stripe-reconcile-failed` and skips that candidate; Step 11's

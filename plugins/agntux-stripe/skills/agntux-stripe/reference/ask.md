# Natural-language query against Stripe

The user typed `/agntux-stripe <words>` where the first token is
not `sync`. They want a live answer about Stripe.
...
may not exist, and that's fine. The Stripe read MCP
...
1. Use these Stripe read MCP tools to investigate:
   search_stripe_resources, fetch_stripe_resources, stripe_api_read, stripe_api_search, stripe_api_details.
...
configured for stripe), reply verbatim:

> "I can't reach Stripe right now. Make sure the
> Stripe connector is enabled in your host's

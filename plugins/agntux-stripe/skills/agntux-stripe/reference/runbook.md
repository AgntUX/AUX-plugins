...
- **Cross-source dedup outcomes** (Step 9) —
  `stripe-merged-into-{existing_id}`,
  `stripe-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `stripe-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `stripe-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `stripe-deferred-orphan: <id>` (the originating
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `stripe-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `stripe-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-stripe/sync.md → errors` with one of

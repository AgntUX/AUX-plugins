...
var REFUND_URI = "ui://agntux-stripe/refund";
var REFUND_LABEL = "Stripe refund";
var EMPTY_REFUND = {
  action_id: "",
  payment_intent_id: "",
  charge_amount: 0,
...
async function handleRefund(args, ctx) {
...
var refundViewTool = {
  descriptor: {
    ...
    description: "Use this to issue a refund for a Stripe payment. Shown when the user wants to refund a charge or payment intent from an AgntUX action item. TRIGGER PHRASES (map verbatim to args \u2014 do not paraphrase): 'open the refund composer for action {id}' \u2192 call with { action_id: id }; 'refund payment for action {id}' \u2192 call with { action_id: id }. For these click-time prompts, pass ONLY action_id. The tool reads the action file's `## Compose payload` body section and lifts payment_intent_id, charge_amount, currency, customer_label, max_refundable, suggested_reason, and open_url from disk. Do NOT pass those fields inline \u2014 any inline value overrides the on-disk payload destructively, producing an empty UI. " + MCP_SUFFIX,
    inputSchema: {
      type: "object",
...
var DISPUTE_URI = "ui://agntux-stripe/dispute";
var DISPUTE_LABEL = "Stripe dispute response";
var EMPTY_DISPUTE = {
  action_id: "",
  dispute_id: "",
  charge_amount: 0,
...
async function handleDispute(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
...
var disputeViewTool = {
  descriptor: {
    ...
    description: "Use this to respond to a Stripe dispute by submitting evidence. Shown when the user needs to reply to a chargeback from an AgntUX action item. TRIGGER PHRASES (map verbatim to args \u2014 do not paraphrase): 'open the dispute evidence composer for action {id}' \u2192 call with { action_id: id }; 'submit dispute evidence for action {id}' \u2192 call with { action_id: id }. For these click-time prompts, pass ONLY action_id. The tool reads the action file's `## Compose payload` body section and lifts dispute_id, charge_amount, currency, dispute_reason, evidence_due_by, draft_evidence, customer_label, and open_url from disk. Do NOT pass those fields inline \u2014 any inline value overrides the on-disk payload destructively, producing an empty UI. " + MCP_SUFFIX,
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var INVOICE_FINALIZE_URI = "ui://agntux-stripe/invoice-finalize";
var INVOICE_FINALIZE_LABEL = "Stripe invoice finalize";
var EMPTY_INVOICE_FINALIZE = {
  action_id: "",
  invoice_id: "",
  invoice_number: "",
  amount_due: 0,
...
async function handleInvoiceFinalize(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
...
var invoiceFinalizeViewTool = {
  descriptor: {
    name: "agntux_stripe_invoice_finalize_view",
    description: "Use this to finalize a Stripe draft invoice, locking it and sending it to the customer. Shown when the user wants to finalize an invoice from an AgntUX action item. TRIGGER PHRASES (map verbatim to args \u2014 do not paraphrase): 'open the invoice finalize composer for action {id}' \u2192 call with { action_id: id }; 'finalize invoice for action {id}' \u2192 call with { action_id: id }. For these click-time prompts, pass ONLY action_id. The tool reads the action file's `## Compose payload` body section and lifts invoice_id, invoice_number, amount_due, currency, customer_label, due_date, and open_url from disk. Do NOT pass those fields inline \u2014 any inline value overrides the on-disk payload destructively, producing an empty UI. " + MCP_SUFFIX,
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var INVOICE_VOID_URI = "ui://agntux-stripe/invoice-void";
var INVOICE_VOID_LABEL = "Stripe invoice void";
var EMPTY_INVOICE_VOID = {
  action_id: "",
  invoice_id: "",
  invoice_number: "",
  amount_due: 0,
...
async function handleInvoiceVoid(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
...
var invoiceVoidViewTool = {
  descriptor: {
    name: "agntux_stripe_invoice_void_view",
    description: "Use this to void a Stripe invoice, marking it uncollectable. This action is irreversible. Shown when the user wants to void an invoice from an AgntUX action item. TRIGGER PHRASES (map verbatim to args \u2014 do not paraphrase): 'open the invoice void composer for action {id}' \u2192 call with { action_id: id }; 'void invoice for action {id}' \u2192 call with { action_id: id }. For these click-time prompts, pass ONLY action_id. The tool reads the action file's `## Compose payload` body section and lifts invoice_id, invoice_number, amount_due, currency, customer_label, status, and open_url from disk. Do NOT pass those fields inline \u2014 any inline value overrides the on-disk payload destructively, producing an empty UI. " + MCP_SUFFIX,
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var SUBSCRIPTION_EDIT_URI = "ui://agntux-stripe/subscription-edit";
var SUBSCRIPTION_EDIT_LABEL = "Stripe subscription edit";
var EMPTY_SUBSCRIPTION_EDIT = {
  action_id: "",
  subscription_id: "",
  customer_label: "",
  current_status: "",
...
async function handleSubscriptionEdit(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
...
var subscriptionEditViewTool = {
  descriptor: {
    name: "agntux_stripe_subscription_edit_view",
    description: "Use this to pause or update a Stripe subscription \u2014 change quantity or pause collection. Shown when the user wants to manage a subscription from an AgntUX action item. TRIGGER PHRASES (map verbatim to args \u2014 do not paraphrase): 'open the subscription edit composer for action {id}' \u2192 call with { action_id: id }; 'edit subscription for action {id}' \u2192 call with { action_id: id }; 'pause subscription for action {id}' \u2192 call with { action_id: id }. For these click-time prompts, pass ONLY action_id. The tool reads the action file's `## Compose payload` body section and lifts subscription_id, customer_label, current_status, current_plan, current_quantity, current_period_end, and open_url from disk. Do NOT pass those fields inline \u2014 any inline value overrides the on-disk payload destructively, producing an empty UI. " + MCP_SUFFIX,
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var SUBSCRIPTION_CANCEL_URI = "ui://agntux-stripe/subscription-cancel";
var SUBSCRIPTION_CANCEL_LABEL = "Stripe subscription cancel";
var EMPTY_SUBSCRIPTION_CANCEL = {
  action_id: "",
  subscription_id: "",
  customer_label: "",
  current_status: "",
...
async function handleSubscriptionCancel(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
...
var subscriptionCancelViewTool = {
  descriptor: {
    name: "agntux_stripe_subscription_cancel_view",
    description: "Use this to cancel a Stripe subscription, either at the end of the current billing period (the default) or immediately. Shown when the user wants to cancel a subscription from an AgntUX action item. TRIGGER PHRASES (map verbatim to args \u2014 do not paraphrase): 'open the subscription cancel composer for action {id}' \u2192 call with { action_id: id }; 'cancel subscription for action {id}' \u2192 call with { action_id: id }. For these click-time prompts, pass ONLY action_id. The tool reads the action file's `## Compose payload` body section and lifts subscription_id, customer_label, current_status, current_period_end, and open_url from disk. Do NOT pass those fields inline \u2014 any inline value overrides the on-disk payload destructively, producing an empty UI. " + MCP_SUFFIX,
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var agntux_stripe_view_default = mod;
export {
  agntux_stripe_view_default as default
};

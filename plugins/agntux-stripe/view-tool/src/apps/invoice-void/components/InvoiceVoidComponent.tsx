...
function parsePayload(toolOutput?: Record<string, unknown>): InvoiceVoidData {
...
export function InvoiceVoidComponent(props: MainComponentProps) {
  ...
  const voidDisabled = isStreaming || sendState !== 'idle' || !data.invoice_id;
  ...
  if (sendState === 'sent') {
    return (
      <ScrollablePanel title="Void invoice">
        <div className="flex flex-col items-center justify-center py-12 text-center px-4">
          <p className="text-sm font-medium text-foreground">Invoice voided.</p>
          <p className="mt-1 text-xs text-muted-foreground">
            The invoice has been marked as void in Stripe.
          </p>
        </div>
      </ScrollablePanel>
    );
      ...
      <ScrollablePanel
        ...
        footer={
          <div className="flex items-center justify-end gap-3 border-t border-border bg-background px-4 py-3">
            ...
            >
              {sendState === 'sending' ? 'Voiding...' : 'Void invoice'}
            </button>
          </div>
        }
      >
        ...
        >
          ...
          <div className="mx-4 mt-4 rounded-md border border-border bg-muted/30 px-3 py-2 space-y-1">
            ...
            {data.status && (
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-foreground capitalize">{data.status}</span>
              </div>
            )}
          </div>
          ...
          <div className="mx-4 mt-3 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2">
            <p className="text-xs font-medium text-destructive">
              This action is irreversible. Voiding an invoice marks it uncollectable and cannot be undone.
            </p>
          </div>
        </div>
      </ScrollablePanel>

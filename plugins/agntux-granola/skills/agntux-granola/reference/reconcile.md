...
the next run the world has moved: the Granola artefact may
...
- **Re-check via** — the `list_meetings, get_meetings, get_meeting_transcript, list_meeting_folders` read call(s) that fetch the
...
to Granola. A re-check failure logs
`granola-reconcile-failed` and skips that candidate; Step 11's

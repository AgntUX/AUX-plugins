# Natural-language query against Granola

The user typed `/agntux-granola <words>` where the first token is
not `sync`. They want a live answer about Granola.
...
may not exist, and that's fine. The Granola read MCP
...
1. Use these Granola read MCP tools to investigate:
   list_meetings, get_meetings, get_meeting_transcript, list_meeting_folders.
...
configured for granola), reply verbatim:

> "I can't reach Granola right now. Make sure the
> Granola connector is enabled in your host's

---
name: agntux-granola
description: The Granola command surface for AgntUX. Runs an ingest pass against Granola or answers natural-language questions about granola content live. Use for "/agntux-granola", "sync granola", "ingest granola now", "refresh granola", "fire a granola pass", "what's happening in #team standup", "what's in my granola", "what did {person} say in granola this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread granola", or any Granola-scoped question.
...
# `/agntux-granola` — Granola command surface

Lane: single user-facing entry into the agntux-granola plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Granola surface of AgntUX. Never reference

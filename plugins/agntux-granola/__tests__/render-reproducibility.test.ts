// render-reproducibility.test.ts — mirrors lint pass 8 (E15) for agntux-granola.
//
// Asserts the rendered tree at skills/agntux-granola/ is byte-identical to what
// node scripts/render-skill.mjs would produce from the committed canonical +
// per-plugin _overrides. Catches "edited the rendered file by hand instead of
// editing the _overrides" — the same regression lint pass 8 catches, but
// surfaced earlier (every push).
//
// The test locates render-skill.mjs via CLAUDE_PLUGIN_ROOT (the installed
// agntux-build bundle root; set by agntux_validate). On a cold developer
// machine without the env var, the test is skipped rather than hard-failing so
// it never blocks a local `vitest run`.
...
function findRenderSkillScript(): string | null {
  ...
  // (works in the contributor sandbox layout where the bundle is a few levels
  // up from the build date dir, but only when the layout happens to match).

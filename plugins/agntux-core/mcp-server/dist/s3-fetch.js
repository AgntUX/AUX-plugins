...
const CACHE_DIR = join(homedir(), ".agntux", ".ui-cache");
...
export function lruSet(key, html) {
...
// Read the signed URL prefix from the licence cache. Falls back to env vars
// when the cache is uninitialised (development / out-of-band runs).
function resolveSignedBase() {
    try {
    ...
    if (!APP_ID_FALLBACK) {
        throw new Error("No signed_ui_base_url in ~/.agntux/.license and no AGNTUX_APP_ID env fallback. " +
...
// path "triage/index.html" becomes
// "https://.../skills/X/triage/index.html?X-Amz-Signature=...".
function appendPathPreservingQuery(base, path) {
...
export async function fetchUIBundle(path) {
...
//# sourceMappingURL=s3-fetch.js.map
...
function errorEnvelope(path, error, message) {
    return {
        isError: true,
...
export const createProjectDirectoryTool = {
    ...
    async handler(_args) {
        ...
        if (inspection.state === "file") {
            return errorEnvelope(path, "path-is-file", `Cannot create the AgntUX project directory: ${path} already exists and is not a directory. Move or remove that file, then retry.`);
        }
        if (inspection.state === "error") {
            return errorEnvelope(path, "stat-failed", `Cannot inspect the AgntUX project directory at ${path}: ${inspection.message}`);
        }
        ...
        if (created) {
            try {
                ...
                return errorEnvelope(path, "mkdir-failed", `Failed to create the AgntUX project directory at ${path}: ${message}`);
            }
        }
        const text = created

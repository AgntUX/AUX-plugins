...
const TRIAGE_STATE_PATH_RE = /^(actions|teams\/[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?\/actions|leader-views\/[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?\/actions)\/[^/\\\0]{1,200}\.md$/;
...
function sanitizeFilterMap(raw) {
    ...
    for (const [k, v] of Object.entries(raw)) {
        if (typeof k !== "string" || !SLUG_RE.test(k))
            continue;
        if (typeof v !== "string" || !VALID_FILTER_STATES.has(v))
            continue;
        out[k] = v;
        count++;
        if (count >= MAX_MUTED_SLUGS)
            break;
    }
    return out;
}
...
function sanitizeTriageState(raw) {
    ...
    for (const [k, v] of Object.entries(raw)) {
        ...
        count++;
        if (count >= MAX_TRIAGE_STATE_ENTRIES)
            break;
    }
    return out;
}
...
export function readTriagePrefs() {
    let raw;
    try {
    ...
    for (const slug of muted_team_slugs) {
        if (team_filters[slug] === undefined)
            team_filters[slug] = "hidden";
    }
    for (const slug of muted_view_slugs) {
        if (view_filters[slug] === undefined)
            view_filters[slug] = "hidden";
    }
...
export const triagePrefsTool = {
    ...
    async handler(args) {
        ...
        if (typeof args.show_done === "boolean")
            next.show_done = args.show_done;
...
export const setTriagePrefTool = {
    ...
    async handler(args) {
        ...
        if (!isValidTriageStatePath(path)) {
            throw new Error(`Path traversal rejected: invalid triage-prefs path "${path}".`);
        }
        const current = readTriagePrefs();
        const next = {
            ...current,

...
export const syncInstalledPluginsTool = {
    ...
    async handler(args) {
        ...
        // plugin's view-tools).
        const isEmptyNoop = rawArg === undefined ||
            rawArg === null ||
        ...
        if (!Array.isArray(rawArg)) {
            const text = `agntux_core_sync_installed_plugins: the \`plugins\` argument must be an array ` +
        ...
        if (sanitized.length === 0) {
            const text = `agntux_core_sync_installed_plugins: received ${rawArg.length} plugin item(s) but ` +

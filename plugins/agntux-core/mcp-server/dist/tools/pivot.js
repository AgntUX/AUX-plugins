...
function guardEntityShape(subtype, slug) {
    if (!SUBTYPE_RE.test(subtype)) {
        throw new Error(`Invalid subtype "${subtype}": must be a kebab-case identifier (lowercase letters, digits, hyphens; starts with a letter; max 64 chars).`);
    }
    if (!SLUG_RE.test(slug)) {
        throw new Error(`Invalid slug "${slug}": must be a kebab-case identifier (lowercase letters, digits, hyphens; starts with a letter; max 64 chars).`);
    }
}
function guardEntityPath(subtype, slug) {
    ...
    if (rel.startsWith("..") || resolve(rel) === rel) {
        throw new Error(`Path traversal rejected: subtype "${subtype}" / slug "${slug}" resolves outside <agntux project root>/entities/`);
    }
}
export const pivotTool = {
    description: "Navigate to an entity view in the orchestrator UI. Returns a host_prompt that opens the entity browser for the specified entity.",
    inputSchema: {
        type: "object",
        properties: {
            subtype: {

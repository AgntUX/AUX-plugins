...
function safeNumberArg(v, fallback) {
...
export const triageViewTool = {
    ...
    inputSchema: {
        ...
        properties: {
            view_handled_days: {
                type: "number",
                description: "Optional. Time window for handled-recent items, in days. Default 7, max 30.",
            },
            limit: {
                type: "number",
                description: "Optional. Cap on the open-actions list. Default 30, max 50.",
            },
        },
        required: [],
    },
...
// ── Handler ──────────────────────────────────────────────────────────────────
export async function handleTriageView(args) {
    const handledDays = clamp(safeNumberArg(args.view_handled_days, DEFAULT_HANDLED_DAYS), 1, MAX_HANDLED_DAYS);
    const limit = clamp(safeNumberArg(args.limit, DEFAULT_LIMIT), 1, MAX_LIMIT);
    ...
    try {
        ...
        return structuredError("actions_index_missing", `triage_view: ${actionsDir} does not exist.`);
    }
    if (!dirStat.isDirectory()) {
        return structuredError("actions_index_missing", `triage_view: ${actionsDir} is not a directory.`);
    }

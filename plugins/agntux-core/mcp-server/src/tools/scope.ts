...
export const SCOPE_INPUT_SCHEMA_FRAGMENT = {
  team_slug: {
    type: "string",
    description:
      "Optional. Route the write to `<root>/teams/{team_slug}/actions/` instead of personal. Mutually exclusive with view_slug. Omit (solo path) for personal items.",
  },
  view_slug: {
    type: "string",
    description:
      "Optional. Route the write to `<root>/leader-views/{view_slug}/actions/` instead of personal. Mutually exclusive with team_slug. Omit for personal items.",
  },
} as const;

export function readScopeFromArgs(args: Record<string, unknown>): ActionScope | undefined {

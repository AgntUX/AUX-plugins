...
const DENY: string[] = ["authorization","cookie","token","secret","password","passwd","jwt","apikey","privatekey","refresh","idtoken","session","credential","bearer","signingkey"];
...
export const scrub = (v: unknown, seen = new WeakSet<object>(), depth = 0): unknown => {
...
export function createSentry({ release, environment, tags }: CreateSentryOptions = {}): SentryClient {
  ...
  const send = (event: SentryEvent): Promise<string | null> =>
  ...
  const baseEvent = (level: SentryLevel): SentryEvent => ({

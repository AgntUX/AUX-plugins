...
2. If no slug is passed, infer from the imperative:
   - "email" / "inbox" → search for installed email plugins (`gmail-ingest`, etc.) by checking `~/agntux/data/schema/contracts/`. If multiple match, ask: "Should this apply to {slug-1} or {slug-2}?" If only one matches, use it.

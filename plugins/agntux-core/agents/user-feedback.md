...
Read `<agntux project root>/data/instructions/{plugin-slug}.md`. If it doesn't exist, create it with the standard frontmatter + four section headings (set `status: final` because a Mode A capture is a finished rule, not a draft):
...
4. Existing `<agntux project root>/data/instructions/{plugin-slug}.md` if present (e.g., a re-run of `/agntux-teach`). Don't overwrite — extend.

...
3. **Migration aid (check this BEFORE creating anything).** If `~/agntux-code/` has data and `~/agntux/` is absent or empty, offer to rename `~/agntux-code` → `~/agntux`; if both are populated, ask which is canonical, emit manual merge steps, and do NOT auto-merge. After a rename, treat the resulting `~/agntux` as the root and go to step 4(b) to request access. If `~/agntux-code/` has no data, fall straight to step 4. (This runs first so the empty `~/agntux` created in step 4 can never spuriously satisfy the "absent or empty" trigger.)
4. Otherwise there's no agntux project yet — **create one, then get working access** (never ask the user to run a terminal command): (a) Call `agntux_core_create_project_directory` (no arguments). It creates `~/agntux` if missing — a no-op if it already exists — and returns the **absolute path**. Cache that absolute string as `<agntux project root>`. If it returns an error (e.g. `~/agntux` already exists as a file), surface the one-line error and stop. (b) Resolve `ToolSearch({query: "select:mcp__cowork__request_cowork_directory", max_results: 1})`; if resolved, call it with `{path: <absolute path from (a)>}` — on approval host re-points cwd, Stage 0 resumes next turn, stop; on user decline tell them they can select the folder in the project picker and re-run, then stop. (c) If `request_cowork_directory` does NOT resolve (non-Cowork host), the directory now exists, so tell the user "I've created your AgntUX project at `{absolute path}`. Open or select that folder, then re-run `/agntux onboard`." and stop. (Last-resort Glob `**/agntux` depth 4 below `os.homedir()` only if step (a) itself failed: 0 results → tell the user "I couldn't create or locate an AgntUX project — create a folder named `agntux` in your home directory and re-run." and stop; 1 result → tell user the path and ask them to select it; 2+ → list numbered, ask which.)

---
name: onboard
description: First-run setup for AgntUX. Creates `~/agntux/user.md` (identity, responsibilities, day-to-day, aspirations, goals, preferences, glossary, sources, plugins) via a guided interview, then bootstraps the tenant schema. Use when the user says "onboard me", "set me up", "get started with AgntUX", "first-time setup", "redo onboarding", "start over", or when any other agntux-core skill detects `user.md` is missing.
---
...
3. **`user.md` already exists?** — if `~/agntux/user.md` already

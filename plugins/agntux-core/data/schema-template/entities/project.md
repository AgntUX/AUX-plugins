...
- `id`, `type: entity`, `schema_version`, `subtype: project`, `aliases`, `sources`, `source`, `source_ref`, `entity_id`, `created_at`, `updated_at`, `last_active`, `deleted_upstream` (see `person.md` for shared semantics, including the hook-computed `entity_id` rule).

...
- Reads Markdown and plain-text files from a configurable local directory (default: `~/agntux-code/notes/`).
...
3. Create your notes directory: `mkdir -p ~/agntux-code/notes/`
...
**Notes directory:** by default the plugin reads from `~/agntux-code/notes/`. To point it at a
...
days. To change this, add `bootstrap_window_days: N` to the frontmatter of `~/agntux-code/user.md`.
Valid range: 1–365.

**Triage preferences:** edit `~/agntux-code/user.md` → `# Preferences` to control which notes

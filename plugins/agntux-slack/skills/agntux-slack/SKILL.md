---
name: agntux-slack
description: The Slack command surface for AgntUX. Runs an ingest pass against Slack or answers natural-language questions about slack content live. Use for "/agntux-slack", "sync slack", "ingest slack now", "refresh slack", "fire a slack pass", "what's happening in #general", "what's in my slack", "what did {person} say in slack this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread slack", or any Slack-scoped question.
...
# `/agntux-slack` — Slack command surface

Lane: single user-facing entry into the agntux-slack plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Slack surface of AgntUX. Never reference

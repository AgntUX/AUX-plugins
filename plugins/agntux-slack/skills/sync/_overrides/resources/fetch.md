...
Call `slack_read_user_profile()` once per run with no arguments. Cache
`user_id` (e.g. `U01ABC`), `email`, `real_name` for entity resolution
and search-query construction. If this call fails (rare), log kind
`auth` to `sync.md → errors` and exit cleanly — without `user_id`, the
discovery search queries cannot be constructed.

### Step 5b — Discovery sweep

Three search queries seed/touch the cursor map. Paginate each until
exhausted or a per-run cap of 5 pages × 20 results = 100 hits.
`slack_search_public_and_private` caps at 20 per call; paginate via
`cursor`.

...
suggested_actions:
  ...
  - label: "Schedule a reply"
    host_prompt: "ux: Use the agntux-slack plugin to open the reply composer in schedule mode for action {id}."
  ...
  - label: "Summarise to canvas"
    host_prompt: "ux: Use the agntux-slack plugin to open the canvas summariser for action {id}."
```
...
The Draft button also accepts the alternative phrasing `ux: Use the
agntux-slack plugin to draft a reply for action {id}.` and the Schedule
button accepts `ux: Use the agntux-slack plugin to draft a reply and
schedule it for action {id}.` — the view tool's description matches
either form.

The canvas summariser additionally matches `ux: Use the agntux-slack
plugin to summarise the thread for action {id}.`

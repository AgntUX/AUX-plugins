 ...
 * @example
 * ```typescript
 * import { getMcpServerConfig, getMcpServerBaseUrl } from "./lib/apps-client";
 *
 ...
 */
declare global {
  // Build-time injected global. Declared with `var` (not `const`) so that
  // when multiple modules in the same tsconfig include both declare this
  // ambient global — which happens in rich-restored plugins that ship
  // per-UI apps-client copies at view-tool/src/apps/{ui}/lib/apps-client/
  // — TypeScript merges the declarations instead of rejecting them with
  // TS2451 ("Cannot redeclare block-scoped variable"). Behaviour at
  // runtime is identical; the global is set once at build time by the
  // vite plugin (esbuild define) or read at runtime from
  // window.__MCP_SERVER_CONFIG__.
  // eslint-disable-next-line no-var
  var __MCP_SERVER_CONFIG__: McpServerConfig | undefined;

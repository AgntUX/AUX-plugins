...
var MAX_MESSAGES_PREVIEW = 8;
var MAX_MESSAGE_EXCERPT_CHARS = 200;
var MAX_EXCERPT_CHARS = 300;
var MAX_TITLE_CHARS = 80;
var MAX_TLDR_CHARS = 500;
var MAX_DECISIONS = 8;
var MAX_DECISION_CHARS = 200;
var MAX_OPEN_QUESTIONS = 8;
var MAX_QUESTION_CHARS = 200;
var MAX_FOLLOWUP_CHARS = 200;
...
function asSlackThreadContext(v) {
  const r = asRecord(v);
  return {
...
function parseSlackCanvasPayload(body) {
...
async function readActionFile(ctx, actionId) {
...
async function handleCompose(args, ctx) {
  ...
  const payload = {
    ...
    thread: {
      ...
      last_reply_excerpt: onDisk.thread_context.last_reply_excerpt ? truncate(onDisk.thread_context.last_reply_excerpt, MAX_EXCERPT_CHARS) : null,
    ...
    messages_truncated: onDisk.thread_context.messages_preview.length > MAX_MESSAGES_PREVIEW,
...
async function handleCanvas(args, ctx) {
  ...
  const payload = {
    ...
    drafted_canvas: {
      ...
      decisions: onDisk.drafted_canvas.decisions.slice(0, MAX_DECISIONS).map((d) => truncate(d, MAX_DECISION_CHARS)),
      open_questions: onDisk.drafted_canvas.open_questions.slice(0, MAX_OPEN_QUESTIONS).map((q) => truncate(q, MAX_QUESTION_CHARS)),
...
var mod = { viewTools: [composeView, canvasView] };
var agntux_slack_view_default = mod;
export {
  agntux_slack_view_default as default
};

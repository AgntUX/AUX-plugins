...
- Shortened the marketplace description to a tighter, benefit-first summary
  (was a long four-sentence paragraph).
...
- **`agntux_slack_compose_view` and `agntux_slack_canvas_view`
  handlers now ship a `content[].text` block alongside
  `structuredContent`** on every return — success and error
  branches alike. Text built from
  `renderConfirmationText("AgntUX Slack reply composer")` /
  `renderConfirmationText("AgntUX Slack canvas summariser")`
  (centralized in `@agntux/plugin-runtime` 0.2.1). Both tools'
  `description` fields were reframed in the same pass: the old
  forbid-list framing ("Once this UI is rendered … do NOT add any
  chat commentary …") became an explanation of the MCP Apps
  lifecycle, and the Stage-2 sentence about the Slack connector's
  send/canvas response is now framed as "surface the connector's
  success/error as plain chat text" rather than a list of forbidden
  re-renders.

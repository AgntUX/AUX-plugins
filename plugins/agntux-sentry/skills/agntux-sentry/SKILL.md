---
name: agntux-sentry
description: The Sentry command surface for AgntUX. Runs an ingest pass against Sentry or answers natural-language questions about sentry content live. Use for "/agntux-sentry", "sync sentry", "ingest sentry now", "refresh sentry", "fire a sentry pass", "what's happening in #web", "what's in my sentry", "what did {person} say in sentry this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread sentry", or any Sentry-scoped question.
...
# `/agntux-sentry` — Sentry command surface

Lane: single user-facing entry into the agntux-sentry plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Sentry surface of AgntUX. Never reference

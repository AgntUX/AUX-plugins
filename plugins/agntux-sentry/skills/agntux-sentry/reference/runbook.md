...
- **Cross-source dedup outcomes** (Step 9) —
  `sentry-merged-into-{existing_id}`,
  `sentry-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `sentry-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `sentry-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `sentry-deferred-orphan: <id>` (the originating
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `sentry-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `sentry-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-sentry/sync.md → errors` with one of

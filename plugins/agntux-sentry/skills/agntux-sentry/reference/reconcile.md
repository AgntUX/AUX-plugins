...
the next run the world has moved: the Sentry artefact may
...
- **Re-check via** — the `find_organizations, find_projects, search_issues, get_sentry_resource, search_events` read call(s) that fetch the
...
to Sentry. A re-check failure logs
`sentry-reconcile-failed` and skips that candidate; Step 11's

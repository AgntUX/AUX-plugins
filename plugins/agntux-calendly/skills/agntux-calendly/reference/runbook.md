# Calendly sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `calendly-merged-into-{existing_id}`,
  `calendly-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `calendly-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `calendly-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `calendly-deferred-orphan: <id>` (the originating
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `calendly-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `calendly-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-calendly/sync.md → errors` with one of

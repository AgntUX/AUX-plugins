...
- **Cross-source dedup outcomes** (Step 9) —
  `canva-merged-into-{existing_id}`,
  `canva-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `canva-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `canva-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `canva-deferred-orphan: <id>` (the originating
  comment's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `canva-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `canva-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-canva/sync.md → errors` with one of

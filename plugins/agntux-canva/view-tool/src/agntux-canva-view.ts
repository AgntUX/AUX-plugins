/**
 * agntux-canva-view.ts — view-tool handler module for agntux-canva.
 *
 * Exports three view tools: reply, comment, export.
 * All write operations are connector-targeted; they go via
 * sendFollowUpMessage() with a connector-envelope instruction string.
 *
 * Handler contract: empty/missing args MUST return a placeholder payload
 * without throwing. The headless render check calls every handler with
 * empty args {}.
 */
...
// ---------------------------------------------------------------------------
// 1. reply — "reply to a comment on a Canva design"
// ---------------------------------------------------------------------------
...
const replyDescriptor: ViewToolDescriptor = {
  name: "agntux_canva_reply",
  description:
    "Renders an inline iframe to reply to a comment on a Canva design. " +
    "Shows the design title, a quoted comment block with the commenter and excerpt, " +
    "and a pre-filled editable reply textarea. Use this when the user wants to " +
    "respond to a comment or mention on one of their Canva designs. " +
...
const replyTool: ViewTool<ReplyArgs, ReplyOut> = {
  descriptor: replyDescriptor,
  async handle(args: ReplyArgs, _ctx: ViewToolContext) {
    const out: ReplyOut = {
...
// ---------------------------------------------------------------------------
// 2. comment — "leave a comment on a Canva design"
// ---------------------------------------------------------------------------
...
const commentDescriptor: ViewToolDescriptor = {
  ...
  description:
    "Renders an inline iframe to leave a new comment on a Canva design. " +
    "Shows the design title and a pre-filled editable comment textarea. " +
    "Use this when the user wants to add a fresh comment to a Canva design " +
...
// ---------------------------------------------------------------------------
// 3. export — "export a Canva design to a shareable file"
// ---------------------------------------------------------------------------
...
function safeFormatArray(value: unknown): string[] {
...
const exportDescriptor: ViewToolDescriptor = {
  name: "agntux_canva_export",
  description:
    "Renders an inline iframe to export a Canva design to a shareable file. " +
    "Shows the design title, a format picker (from the design's supported formats), " +
    "and a page range control. The user selects format and pages then clicks Export. " +
    "Use this when the user wants to download or share a Canva design as PDF, PNG, " +
    "JPG, PPTX, GIF, MP4, or CSV. " +
    "This tool is an MCP App view tool: it returns a structured data " +
    "payload that the host (Claude Desktop / Claude Cowork / Claude Code) " +
    "renders into an interactive iframe shown above the next assistant " +
    "turn. The iframe is the user-visible result of calling this tool; " +
...
const exportTool: ViewTool<ExportArgs, ExportOut> = {
  descriptor: exportDescriptor,
  async handle(args: ExportArgs, _ctx: ViewToolContext) {
    ...
    const out: ExportOut = {
      ...
      default_format:
        typeof args.default_format === "string"
          ? args.default_format
          : formats.length > 0
          ? formats[0]
          : "pdf",

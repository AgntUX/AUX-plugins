---
name: agntux-gmail
description: The Gmail command surface for AgntUX. Runs an ingest pass against Gmail or answers natural-language questions about gmail content live. Use for "/agntux-gmail", "sync gmail", "ingest gmail now", "refresh gmail", "fire a gmail pass", "what's happening in #Inbox", "what's in my gmail", "what did {person} say in gmail this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread gmail", or any Gmail-scoped question.
...
# `/agntux-gmail` — Gmail command surface

Lane: single user-facing entry into the agntux-gmail plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Gmail surface of AgntUX. Never reference

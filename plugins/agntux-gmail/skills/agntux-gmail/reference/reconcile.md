...
the next run the world has moved: the Gmail artefact may
...
- **Re-check via** — the `search_threads, get_thread, list_labels (read-only); the write tools create_label, create_draft are inherited but forbidden by this prompt — view-tool send envelopes own the draft path` read call(s) that fetch the
...
to Gmail. A re-check failure logs
`gmail-reconcile-failed` and skips that candidate; Step 11's

...
the next run the world has moved: the DocuSign artefact may
...
DocuSign specifics by shipping
...
- **Re-check via** — the `getEnvelopes, getEnvelope, listRecipients, listEnvelopeDocuments, getAllAgreements, getAgreementDetails, getTemplates` read call(s) that fetch the
...
to DocuSign. A re-check failure logs
`docusign-reconcile-failed` and skips that candidate; Step 11's

# DocuSign sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `docusign-merged-into-{existing_id}`,
  `docusign-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `docusign-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `docusign-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `docusign-deferred-orphan: <id>` (the originating
  envelope's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `docusign-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `docusign-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-docusign/sync.md → errors` with one of

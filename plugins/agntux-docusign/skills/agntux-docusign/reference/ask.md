# Natural-language query against DocuSign

The user typed `/agntux-docusign <words>` where the first token is
not `sync`. They want a live answer about DocuSign.
...
may not exist, and that's fine. The DocuSign read MCP
...
1. Use these DocuSign read MCP tools to investigate:
   getEnvelopes, getEnvelope, listRecipients, listEnvelopeDocuments, getAllAgreements, getAgreementDetails, getTemplates.
...
configured for docusign), reply verbatim:

> "I can't reach DocuSign right now. Make sure the
> DocuSign connector is enabled in your host's

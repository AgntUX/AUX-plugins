---
name: agntux-apple-notes
description: The Apple Notes command surface for AgntUX. Runs an ingest pass against Apple Notes or answers natural-language questions about apple-notes content live. Use for "/agntux-apple-notes", "sync apple-notes", "ingest apple-notes now", "refresh apple-notes", "fire a apple-notes pass", "what's happening in #Personal", "what's in my apple-notes", "what did {person} say in apple-notes this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread apple-notes", or any Apple Notes-scoped question.
...
# `/agntux-apple-notes` — Apple Notes command surface

Lane: single user-facing entry into the agntux-apple-notes plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Apple Notes surface of AgntUX. Never reference

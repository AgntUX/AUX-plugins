# Apple Notes sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `apple-notes-merged-into-{existing_id}`,
  `apple-notes-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `apple-notes-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `apple-notes-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `apple-notes-deferred-orphan: <id>` (the originating
  note's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `apple-notes-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `apple-notes-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-apple-notes/sync.md → errors` with one of

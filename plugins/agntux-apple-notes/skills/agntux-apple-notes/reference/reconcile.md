...
the next run the world has moved: the Apple Notes artefact may
...
Apple Notes specifics by shipping
...
- **Re-check via** — the `mcp__Read_and_Write_Apple_Notes__list_notes, mcp__Read_and_Write_Apple_Notes__get_note_content` read call(s) that fetch the
...
to Apple Notes. A re-check failure logs
`apple-notes-reconcile-failed` and skips that candidate; Step 11's

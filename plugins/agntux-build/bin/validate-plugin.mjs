 ...
 *
 * Usage:
 *   node scripts/validate-plugin.mjs <slug>                       (maintainer clone)
 *   node bin/validate-plugin.mjs <slug> --plugin-dir <abs>        (contributor bundle)
 ...
 *   --plugin-dir    absolute path to the plugin tree to BUILD, LINT, and hash.
 *                   In the maintainer clone this defaults to
 *                   <REPO_ROOT>/plugins/<slug>. In the contributor bundle there
 *                   is no clone, so --plugin-dir is REQUIRED and points at the
 *                   build sandbox tree
 ...
 *
 * The build + lint steps run the bundled toolchain (build-plugin.mjs,
 * lint-marketplace-metadata.ts) against the EXACT --plugin-dir tree — not a
 * fixed clone path — so the bytes that are built+linted are the bytes that get
 * hashed into the receipt and submitted. The toolchain (scripts, built
 * packages, canonical templates) is resolved via toolchain-layout.mjs so the
 * same code runs from the maintainer clone (<repo>/scripts) and the contributor
 * bundle (<plugin>/bin + <plugin>/scripts) alike.
 *
...
async function main() {
  ...
  // The tree to build, lint, and hash. The build + lint steps now target this
  // exact --plugin-dir (no fixed clone path), so the validated bytes ARE the
  // hashed/submitted bytes — there is nothing to keep "in sync" and no guard to
  // forge past. In the maintainer clone --plugin-dir defaults to
  // <repo>/plugins/<slug>; in the contributor bundle there is no clone, so it
  // is required.
  let pluginDir;
  if (argv.pluginDir) {
  ...
  {
    // Build the EXACT --plugin-dir tree. AGNTUX_PACKAGES_DIR points the build's
    // package auto-resolution at the bundled (or repo) packages so
    // @agntux/ui-primitives resolves with no clone present.
  ...
  // ── 2. lint (HARD) ────────────────────────────────────────────────────────
  // The marketplace linter, run against the EXACT --plugin-dir tree. The
  // canonical-root / apps-client-canonical-root / tmp-root flags let the
  // canonical-coupled passes (8 render-repro, 12 apps-client drift) resolve
  // their canonical sources + a writable scratch dir whether we're in the
  // maintainer clone (everything under <repo>) or the contributor bundle
  // (canonical templates bundled, repo-mirror for apps-client, OS tmp scratch).
  log("[2/6] lint (marketplace linter --plugin-dir)");
  {
    ...
    if (r.status !== 0) {
      ...
      // manifest-author, pass-8/E15 → ingest-prompt-author, BUILD-* → view-tool-builder.
      const codes = (r.stdout + r.stderr).match(/\b(E\d{2}|BUILD-[A-Za-z-]+)\b/g) || [];
      fail("lint", `marketplace lint failed for ${slug}${codes.length ? ` (codes: ${[...new Set(codes)].join(",")})` : ""}`);
    }
  }
...
function readAgntuxBuildVersion() {
  // Prefer the running plugin context; fall back to the bundle root, then the
  // maintainer clone's tracked plugin.json.
  const candidates = [

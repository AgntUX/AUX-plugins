...
- **A backing view tool means a MANDATORY hand-off, never a plain-text reply.** Any skill lane or model hand-off that presents data which has a backing view tool MUST use MANDATORY language that (i) names the EXACT view-tool id to call — the literal `agntux_<slug>_<name>` id, never "the schedule view" or "the results view" — and (ii) explicitly FORBIDS answering in plain text or a chat summary, even for a single result or a case that feels "simple." Canonical phrasing: `You MUST call agntux_<slug>_<name> with <data>. Do NOT list/summarise the results as text — the view is the only correct surface.` Soft phrasing ("re-open the view", "consider showing the times") lets the model take the text path — that is the agntux-google-calendar time-slot bug, where the view never opened and the model dumped a plain-text list of times into chat instead of calling the view tool

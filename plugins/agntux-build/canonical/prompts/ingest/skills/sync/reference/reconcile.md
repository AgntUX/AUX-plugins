...
the next run the world has moved: the {{source-display-name}} artefact may
...
{{source-display-name}} specifics by shipping
...
- **Re-check via** — the `{{source-mcp-tools}}` read call(s) that fetch the
...
to {{source-display-name}}. A re-check failure logs
`{{source-slug}}-reconcile-failed` and skips that candidate; Step 11's

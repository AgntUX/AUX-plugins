---
name: agntux-asana
description: The Asana command surface for AgntUX. Runs an ingest pass against Asana or answers natural-language questions about asana content live. Use for "/agntux-asana", "sync asana", "ingest asana now", "refresh asana", "fire a asana pass", "what's happening in #my-tasks", "what's in my asana", "what did {person} say in asana this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread asana", or any Asana-scoped question.
...
# `/agntux-asana` — Asana command surface

Lane: single user-facing entry into the agntux-asana plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Asana surface of AgntUX. Never reference

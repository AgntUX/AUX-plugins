...
- **Cross-source dedup outcomes** (Step 9) —
  `asana-merged-into-{existing_id}`,
  `asana-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `asana-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `asana-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `asana-deferred-orphan: <id>` (the originating
  task's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `asana-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `asana-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-asana/sync.md → errors` with one of

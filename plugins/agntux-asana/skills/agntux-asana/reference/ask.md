# Natural-language query against Asana

The user typed `/agntux-asana <words>` where the first token is
not `sync`. They want a live answer about Asana.
...
may not exist, and that's fine. The Asana read MCP
...
1. Use these Asana read MCP tools to investigate:
   get_my_tasks, get_task, get_tasks, get_projects, get_project, get_status_overview, get_users, get_user, search_tasks.
...
configured for asana), reply verbatim:

> "I can't reach Asana right now. Make sure the
> Asana connector is enabled in your host's

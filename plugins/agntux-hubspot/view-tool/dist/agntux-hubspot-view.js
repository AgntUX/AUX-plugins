...
function parseYamlSection(body, header) {
...
var CONTENT_TEXT = (label) => [
  { type: "text", text: renderConfirmationText(label) }
];
function errorPayload(error) {
  return { error };
}
var MOVE_DEAL_RESOURCE_URI = "ui://agntux-hubspot/move-deal";
var MOVE_DEAL_LABEL = "HubSpot move deal";
async function handleMoveDeal(args, ctx) {
  ...
  try {
    ...
    const available_stages = rawStages.map((x) => {
      if (!x || typeof x !== "object") return null;
      const r = x;
      const id = str2(r.id);
      if (!id) return null;
      return { id, label: str2(r.label) };
    }).filter((s) => s !== null);
    const payload = {
      deal_url: str2(data.deal_url),
      deal_id: str2(data.deal_id),
      deal_name: str2(data.deal_name),
      pipeline_label: str2(data.pipeline_label),
      current_stage: str2(data.current_stage),
      available_stages,
      amount: str2(data.amount),
      currency_code: str2(data.currency_code),
      close_date: str2(data.close_date)
    };
    return {
      content: CONTENT_TEXT(MOVE_DEAL_LABEL),
...
var TASK_RESOURCE_URI = "ui://agntux-hubspot/task";
var TASK_LABEL = "HubSpot task";
async function handleTask(args, ctx) {
  ...
  try {
    ...
    const payload = {
      task_url: str2(data.task_url),
      task_id: str2(data.task_id),
      task_title: str2(data.task_title),
      due_date: str2(data.due_date),
      status: str2(data.status),
      associated_record_name: str2(data.associated_record_name),
...
var taskViewTool = {
  descriptor: {
    ...
    description: "Use this to complete or reschedule a HubSpot task on behalf of the user. Opens a task card with tabs to mark the task complete or pick a new due date. This tool is an MCP App view tool: it returns a structured data payload that the host (Claude Desktop / Claude Cowork / Claude Code) renders into an interactive iframe shown above the next assistant turn. The iframe is the user-visible result of calling this tool; no additional chat output, summary, or visualization tool call is needed afterwards.",
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var ACTIVITY_RESOURCE_URI = "ui://agntux-hubspot/activity";
var ACTIVITY_LABEL = "HubSpot log activity";
var DRAFT_BODY_MAX = 2e3;
async function handleActivity(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
  ...
  try {
    ...
    let draftBody = str2(data.draft_body);
    if (draftBody.length > DRAFT_BODY_MAX) {
      draftBody = draftBody.slice(0, DRAFT_BODY_MAX);
    }
    const payload = {
      record_url: str2(data.record_url),
      record_id: str2(data.record_id),
      record_type: str2(data.record_type),
      record_name: str2(data.record_name),
...
var activityViewTool = {
  descriptor: {
    name: "agntux_hubspot_activity_view",
    description: "Use this to log a note or activity on a HubSpot record on behalf of the user. Opens a note composer pre-filled with a draft body the user can edit before submitting. This tool is an MCP App view tool: it returns a structured data payload that the host (Claude Desktop / Claude Cowork / Claude Code) renders into an interactive iframe shown above the next assistant turn. The iframe is the user-visible result of calling this tool; no additional chat output, summary, or visualization tool call is needed afterwards.",
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var REASSIGN_RESOURCE_URI = "ui://agntux-hubspot/reassign";
var REASSIGN_LABEL = "HubSpot reassign record";
async function handleReassign(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
  ...
  try {
    ...
    const candidate_owners = rawOwners.map((x) => {
      if (!x || typeof x !== "object") return null;
      const r = x;
      const ownerId = str2(r.ownerId);
      if (!ownerId) return null;
      return { ownerId, name: str2(r.name) };
    }).filter((o) => o !== null);
    const payload = {
      record_url: str2(data.record_url),
      record_id: str2(data.record_id),
      record_type: str2(data.record_type),
      record_name: str2(data.record_name),
      current_owner: str2(data.current_owner),
      candidate_owners
    };
    return {
      content: CONTENT_TEXT(REASSIGN_LABEL),
...
var reassignViewTool = {
  descriptor: {
    name: "agntux_hubspot_reassign_view",
    description: "Use this to reassign a HubSpot record (deal, ticket, or contact) to another owner on behalf of the user. Opens an owner picker with team members available to receive the record. This tool is an MCP App view tool: it returns a structured data payload that the host (Claude Desktop / Claude Cowork / Claude Code) renders into an interactive iframe shown above the next assistant turn. The iframe is the user-visible result of calling this tool; no additional chat output, summary, or visualization tool call is needed afterwards.",
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var agntux_hubspot_view_default = mod;
export {
  agntux_hubspot_view_default as default
};

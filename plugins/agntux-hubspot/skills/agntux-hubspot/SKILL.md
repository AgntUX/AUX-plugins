---
name: agntux-hubspot
description: The HubSpot command surface for AgntUX. Runs an ingest pass against HubSpot or answers natural-language questions about hubspot content live. Use for "/agntux-hubspot", "sync hubspot", "ingest hubspot now", "refresh hubspot", "fire a hubspot pass", "what's happening in #deals", "what's in my hubspot", "what did {person} say in hubspot this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread hubspot", or any HubSpot-scoped question.
...
# `/agntux-hubspot` — HubSpot command surface

Lane: single user-facing entry into the agntux-hubspot plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the HubSpot surface of AgntUX. Never reference

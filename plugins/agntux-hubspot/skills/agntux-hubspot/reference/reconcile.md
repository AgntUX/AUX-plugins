...
the next run the world has moved: the HubSpot artefact may
...
- **Re-check via** — the `search_crm_objects, get_crm_objects, query_crm_data, search_properties, search_owners` read call(s) that fetch the
...
to HubSpot. A re-check failure logs
`hubspot-reconcile-failed` and skips that candidate; Step 11's

# Natural-language query against HubSpot

The user typed `/agntux-hubspot <words>` where the first token is
not `sync`. They want a live answer about HubSpot.
...
may not exist, and that's fine. The HubSpot read MCP
...
1. Use these HubSpot read MCP tools to investigate:
   search_crm_objects, get_crm_objects, query_crm_data, search_properties, search_owners.
...
configured for hubspot), reply verbatim:

> "I can't reach HubSpot right now. Make sure the
> HubSpot connector is enabled in your host's

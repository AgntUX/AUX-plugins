# HubSpot sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `hubspot-merged-into-{existing_id}`,
  `hubspot-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `hubspot-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `hubspot-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `hubspot-deferred-orphan: <id>` (the originating
  record's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `hubspot-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `hubspot-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-hubspot/sync.md → errors` with one of

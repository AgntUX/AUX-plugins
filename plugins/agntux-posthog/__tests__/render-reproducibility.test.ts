// =============================================================================
// render-reproducibility.test.ts — lint pass 8 mirror for agntux-posthog.
//
// Asserts that the rendered skill tree at skills/agntux-posthog/ is byte-
...
//   - The rendered SKILL.md has not been built yet.
//
// The CORRECT edit surface for per-plugin guidance is:
//   skills/agntux-posthog/_overrides/{step-id}-append.md   (step appends)
//   skills/agntux-posthog/_overrides/reference/{name}.md   (wholesale overrides)
//   skills/agntux-posthog/_overrides/frontmatter.yaml      (substitutions)
//
// NEVER edit the rendered skills/agntux-posthog/SKILL.md or
// skills/agntux-posthog/reference/*.md directly.
// =============================================================================
...
describe("render reproducibility (lint pass 8 mirror)", () => {
  it(
    ...
    async () => {
      if (!renderScriptAvailable()) {
        // Canonical render script not available in this build environment.
        // The gate's lint pass 8 runs in the full monorepo; here we skip.
        console.warn(
      ...
      if (!renderedSkillExists()) {
        // Skill has not been rendered yet (pre-build gate pass).
        console.warn(
          "[render-reproducibility] skills/agntux-posthog/SKILL.md not yet built — skipping.",
        );
        return;
      }

      // Dynamic import — .mjs has no .d.ts; ts-expect-error suppresses the type gap.
  ...
  it("_overrides/ contains both wholesale reference overrides for this plugin", () => {
    // cursor.md and fetch.md are both wholesale overrides (not step appends)
    // for agntux-posthog. Asserted by listing the actual committed files.
  ...
  it("plugin.json name field matches expected slug (cross-checks substitution source)", () => {
    ...
    // Content assertions on _overrides/*.yaml are E30-blocked; plugin.json
    // is the correct grounding for the plugin-slug identity check.
    const m = JSON.parse(

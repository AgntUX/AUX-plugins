# PostHog sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `posthog-merged-into-{existing_id}`,
  `posthog-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `posthog-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `posthog-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `posthog-deferred-orphan: <id>` (the originating
  issue's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `posthog-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `posthog-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-posthog/sync.md → errors` with one of

---
name: agntux-posthog
description: The PostHog command surface for AgntUX. Runs an ingest pass against PostHog or answers natural-language questions about posthog content live. Use for "/agntux-posthog", "sync posthog", "ingest posthog now", "refresh posthog", "fire a posthog pass", "what's happening in #my-project", "what's in my posthog", "what did {person} say in posthog this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread posthog", or any PostHog-scoped question.
...
# `/agntux-posthog` — PostHog command surface

Lane: single user-facing entry into the agntux-posthog plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the PostHog surface of AgntUX. Never reference

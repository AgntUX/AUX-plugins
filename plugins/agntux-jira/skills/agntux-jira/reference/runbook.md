# Jira (Atlassian Cloud) sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `jira-merged-into-{existing_id}`,
  `jira-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `jira-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `jira-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `jira-deferred-orphan: <id>` (the originating
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `jira-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `jira-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-jira/sync.md → errors` with one of

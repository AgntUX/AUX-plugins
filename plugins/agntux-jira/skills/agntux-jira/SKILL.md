---
name: agntux-jira
description: The Jira (Atlassian Cloud) command surface for AgntUX. Runs an ingest pass against Jira (Atlassian Cloud) or answers natural-language questions about jira content live. Use for "/agntux-jira", "sync jira", "ingest jira now", "refresh jira", "fire a jira pass", "what's happening in #OFM", "what's in my jira", "what did {person} say in jira this week", "summarise last week's {topic} thread", "any new threads about {project}", "any unread jira", or any Jira (Atlassian Cloud)-scoped question.
...
# `/agntux-jira` — Jira (Atlassian Cloud) command surface

Lane: single user-facing entry into the agntux-jira plugin. Read
the first whitespace-delimited token of `$ARGUMENTS`; route to the
right reference file.

## Voice rules

Speak as the Jira (Atlassian Cloud) surface of AgntUX. Never reference

/**
 * render-reproducibility.test.ts — mirrors lint pass 8 for agntux-jira.
 *
 * Asserts that the committed `skills/agntux-jira/` tree is byte-identical to
 * what `renderSkill(...)` would produce from canonical + _overrides. Catches
 * "edited the rendered file by hand instead of editing the override" regressions.
 *
 * The rendered tree is expected to exist in the build tree when tests run
 * (the build step calls renderSkill before vitest). If the tree is absent,
 * the test fails fast with a clear message directing the author to run the
 * build step first.
 */
...
async function importRenderSkill(): Promise<{
  ...
  try {
    ...
    return mod as { renderSkill: (o: { canonicalDir: string; overridesDir: string; outputDir: string }) => void; RenderSkillError: new (...args: unknown[]) => Error };
  } catch {
    return null;
  }
}
...
describe("render-reproducibility pre-conditions", () => {
  ...
  it("committed skills/agntux-jira/ tree exists (build step must run first)", () => {
    expect(
      existsSync(join(COMMITTED, "SKILL.md")),
      "skills/agntux-jira/SKILL.md is absent — run the build step first (node scripts/build-plugin.mjs agntux-jira)",
    ).toBe(true);
  });
});
...
describe("render reproducibility (lint pass 8 mirror)", () => {
  it("re-running renderSkill produces output byte-identical to the committed tree", async () => {
    ...
    if (!existsSync(join(COMMITTED, "SKILL.md"))) {
      throw new Error(
        "skills/agntux-jira/SKILL.md is absent — run `node scripts/build-plugin.mjs agntux-jira` first",
      );
    }

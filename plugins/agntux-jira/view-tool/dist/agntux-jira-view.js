...
var COMMENT_RESOURCE_URI = "ui://agntux-jira/comment";
var COMMENT_LABEL = "Jira comment composer";
...
var TRANSITION_RESOURCE_URI = "ui://agntux-jira/transition";
var TRANSITION_LABEL = "Jira issue transition";
async function handleTransition(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
  ...
  try {
    ...
    const rawTransitions = Array.isArray(data.available_transitions) ? data.available_transitions : [];
    const available_transitions = rawTransitions.map((x) => {
...
var transitionViewTool = {
  descriptor: {
    name: "agntux_jira_transition_view",
    description: "Use this to change the status of a Jira issue (e.g., move from In Progress to In Review or Done). Opens a transition picker showing all available transitions with the suggested one pre-selected. This tool is an MCP App view tool: it returns a structured data payload that the host (Claude Desktop / Claude Cowork / Claude Code) renders into an interactive iframe shown above the next assistant turn. The iframe is the user-visible result of calling this tool; no additional chat output, summary, or visualization tool call is needed afterwards.",
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var ASSIGN_RESOURCE_URI = "ui://agntux-jira/assign";
var ASSIGN_LABEL = "Jira issue assign";
async function handleAssign(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
  ...
  try {
    ...
    const candidate_assignees = rawCandidates.map((x) => {
      if (!x || typeof x !== "object") return null;
      const r = x;
...
var assignViewTool = {
  descriptor: {
    ...
    description: "Use this to assign or re-assign a Jira issue to a person. Opens an assignee picker with candidates pre-loaded and the suggested pick pre-selected. This tool is an MCP App view tool: it returns a structured data payload that the host (Claude Desktop / Claude Cowork / Claude Code) renders into an interactive iframe shown above the next assistant turn. The iframe is the user-visible result of calling this tool; no additional chat output, summary, or visualization tool call is needed afterwards.",
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var EDIT_RESOURCE_URI = "ui://agntux-jira/edit";
var EDIT_LABEL = "Jira issue editor";
async function handleEdit(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
  ...
  try {
    ...
    const draft_labels = Array.isArray(draft_labels_raw) ? draft_labels_raw.filter((x) => typeof x === "string") : null;
    const payload = {
      cloud_id: str(data.cloud_id),
      issue_key: str(data.issue_key),
      issue_url: str(data.issue_url),
...
var editViewTool = {
  descriptor: {
    ...
    description: "Use this to edit the fields of a Jira issue \u2014 summary, priority, and labels. Opens an edit form pre-filled with draft suggestions; only changed fields are sent. This tool is an MCP App view tool: it returns a structured data payload that the host (Claude Desktop / Claude Cowork / Claude Code) renders into an interactive iframe shown above the next assistant turn. The iframe is the user-visible result of calling this tool; no additional chat output, summary, or visualization tool call is needed afterwards.",
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var LOG_WORK_RESOURCE_URI = "ui://agntux-jira/log-work";
var LOG_WORK_LABEL = "Jira log work";
async function handleLogWork(args, ctx) {
  const actionId = typeof args.action_id === "string" ? args.action_id : "";
  if (!actionId) {
    return {
...
var logWorkViewTool = {
  descriptor: {
    ...
    description: "Use this to log work time spent on a Jira issue. Opens a worklog form pre-filled with a suggested time estimate and start time. This tool is an MCP App view tool: it returns a structured data payload that the host (Claude Desktop / Claude Cowork / Claude Code) renders into an interactive iframe shown above the next assistant turn. The iframe is the user-visible result of calling this tool; no additional chat output, summary, or visualization tool call is needed afterwards.",
    inputSchema: {
      type: "object",
      properties: { action_id: { type: "string" } },
      required: ["action_id"],
...
var agntux_jira_view_default = mod;
export {
  agntux_jira_view_default as default
};

# Natural-language query against Zoom

The user typed `/agntux-zoom <words>` where the first token is
not `sync`. They want a live answer about Zoom.
...
may not exist, and that's fine. The Zoom read MCP
...
1. Use these Zoom read MCP tools to investigate:
   search_meetings, recordings_list, get_meeting_assets, get_recording_resource, search_zoom, get_file_content.
...
configured for zoom), reply verbatim:

> "I can't reach Zoom right now. Make sure the
> Zoom connector is enabled in your host's

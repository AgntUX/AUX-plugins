...
- **Cross-source dedup outcomes** (Step 9) —
  `zoom-merged-into-{existing_id}`,
  `zoom-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `zoom-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `zoom-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `zoom-deferred-orphan: <id>` (the originating
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `zoom-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `zoom-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-zoom/sync.md → errors` with one of

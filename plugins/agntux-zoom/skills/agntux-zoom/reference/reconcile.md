...
the next run the world has moved: the Zoom artefact may
...
- **Re-check via** — the `search_meetings, recordings_list, get_meeting_assets, get_recording_resource, search_zoom, get_file_content` read call(s) that fetch the
...
to Zoom. A re-check failure logs
`zoom-reconcile-failed` and skips that candidate; Step 11's

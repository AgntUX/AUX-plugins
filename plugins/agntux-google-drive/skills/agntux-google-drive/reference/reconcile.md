...
the next run the world has moved: the Google Drive artefact may
...
Google Drive specifics by shipping
...
- **Re-check via** — the `list_recent_files, search_files, read_file_content, get_file_metadata, get_file_permissions` read call(s) that fetch the
...
to Google Drive. A re-check failure logs
`google-drive-reconcile-failed` and skips that candidate; Step 11's

# Google Drive sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `google-drive-merged-into-{existing_id}`,
  `google-drive-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `google-drive-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `google-drive-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `google-drive-deferred-orphan: <id>` (the originating
  file's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `google-drive-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `google-drive-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-google-drive/sync.md → errors` with one of

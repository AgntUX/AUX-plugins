# Natural-language query against Google Drive

The user typed `/agntux-google-drive <words>` where the first token is
not `sync`. They want a live answer about Google Drive.
...
may not exist, and that's fine. The Google Drive read MCP
...
1. Use these Google Drive read MCP tools to investigate:
   list_recent_files, search_files, read_file_content, get_file_metadata, get_file_permissions.
...
configured for google-drive), reply verbatim:

> "I can't reach Google Drive right now. Make sure the
> Google Drive connector is enabled in your host's

# Google Calendar sync — runbook reference
...
- **Cross-source dedup outcomes** (Step 9) —
  `google-calendar-merged-into-{existing_id}`,
  `google-calendar-reconcile-failed`.
- **Cursor / fetch outcomes** —
  `google-calendar-cursor-evicted: <key>` (third consecutive deleted /
  permission-revoked failure on the same key — see "Failure modes"
  below), `google-calendar-tool-result-truncated` (oversized response
  redirected to a temp file).
- **Deferred-bootstrap outcomes** (Step 8.6) —
  `google-calendar-deferred-orphan: <id>` (the originating
  event's cursor was evicted between the deferred run
  and this drain pass).
...
- **Item deleted / permission revoked** → `kind: source`. On the third
  ...
  log a `google-calendar-cursor-evicted: <key>` entry.
...
- **Tool-result truncation** (the host's MCP layer redirects oversized
  ...
  log `google-calendar-tool-result-truncated` with `kind: source` and
...
Log to `data/learnings/agntux-google-calendar/sync.md → errors` with one of

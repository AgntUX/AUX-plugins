...
the next run the world has moved: the Google Calendar artefact may
...
Google Calendar specifics by shipping
...
- **Re-check via** — the `list_calendars, list_events, get_event` read call(s) that fetch the
...
to Google Calendar. A re-check failure logs
`google-calendar-reconcile-failed` and skips that candidate; Step 11's

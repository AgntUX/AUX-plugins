...
export function createLocalFsContext(
  ...
  const fs: ViewToolFs = {
    ...
    async writeFile(path, body, opts): Promise<WriteResult> {
      const abs = resolveSafe(path);
      const bodyBuf =
      ...
      if (opts && Object.hasOwn(opts, "parent_sha")) {
        ...
        if (current !== expected) {
          throw new ViewToolFsError(
            "conflict",
            path,
            `parent_sha mismatch (expected ${expected ?? "null"}, found ${current ?? "null"})`,
          );
        }
      }
      try {
    ...
    async update(path, patch: UpdatePatch): Promise<WriteResult> {
      for (let attempt = 0; attempt < MAX_UPDATE_RETRIES; attempt++) {
        ...
        try {
          ...
          if (
            err instanceof ViewToolFsError &&
            err.code === "not-found"
          ) {
            current = null;
            parentSha = null;
      ...
      throw new ViewToolFsError(
        "conflict",
        path,
        "update() exceeded retry budget",
      );
    },

    async deleteFile(path, opts): Promise<WriteResult> {
      const abs = resolveSafe(path);
      if (opts && Object.hasOwn(opts, "parent_sha")) {
        const expected = opts.parent_sha;
        const current = await currentShaOrNull(abs);
  ...
  async function currentShaOrNull(abs: string): Promise<string | null> {
    try {
